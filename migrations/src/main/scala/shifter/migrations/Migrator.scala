...
abstract class Migrator(val packageName: String, val group: String) {
  ...
  def migrateTo(version: Int): Boolean = withSetup {
  ...
  def migrateOneDown(): Boolean = withSetup {
  ...
  def migrateOneUp(): Boolean = withSetup {
    nextMigration match {
      case Some(m) => 
	goUp(m)
	persistVersion(m.version)
  ...
  def migrateToZero(): Boolean = withSetup {
    @annotation.tailrec
    def iter(ret: Boolean): Boolean = 
      if (migrateOneDown())
	iter(true)
  ...
  protected def withSetup[A](f: => A): A = {
    setup()
    if (currentVersion > 0)
